...
class StationListConverterTest {
    ...
    @Test
    fun testListToJson() {
        every { gson.toJson(listOf(STATION_ENTITY)) } returns JSON_STATION_ENTITY
        val json = converter.listToJson(listOf(STATION_ENTITY))
        assert(json == JSON_STATION_ENTITY)
    }
    ...
    fun testJsonToList() {
        ...
        assert(json == listOf(STATION_ENTITY))
    }
}